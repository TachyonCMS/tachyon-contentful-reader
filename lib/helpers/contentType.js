...
function fields(root, data) {
   ...
   data.forEach(element => {
      ...
      if(el.required) {
         reqFields.push(element.id)
      }

      if(el.localized) {
         localizedFields.push(element.id)
      }
   });

   if(reqFields.length > 0) {
      const reqFileName = path.join(root, 'sys-required.json')
      out[reqFileName] = { fields: reqFields }
   }

   if(localizedFields.length > 0) {
      const localizedFileName = path.join(root, 'sys-localized.json')
      out[localizedFileName] = { fields: localizedFields }
   }
